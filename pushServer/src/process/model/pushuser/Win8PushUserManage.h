#ifndef _INCLUDE_WIN8_USER_MANAGE_H
#define _INCLUDE_WIN8_USER_MANAGE_H
...
class CWin8PushUserManage : public CStaticNodeManage<CWin8PushUser>
{
public:
	CWin8PushUserManage();
	~CWin8PushUserManage();
...
protected:
	// 更新设置新节点
	const int UpdateSetNode(const int HeadIndex, const CWin8PushUser &Node);
	// 节点(组)是否有效
...
public:
	...
	// 获取用户
	CWin8PushUser* GetUser(const unsigned int);
	// 更新占用的用户数属性
	const short UpdateOccupyProperty(const unsigned int);
	// 更新已经占用的预警资源属性
	const short UpdateUseWarnProperty(const unsigned int);
	// 从数据库加载用户
	const int TryReadUserFromDb(const char *, const unsigned char, CWin8PushUser*);
	// 同步用户基本信息头
...
#endif	/* _INCLUDE_WIN8_USER_MANAGE_H */
