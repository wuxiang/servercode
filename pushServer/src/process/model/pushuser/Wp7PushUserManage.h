#ifndef _INCLUDE_WP7_USER_MANAGE_H
#define _INCLUDE_WP7_USER_MANAGE_H
...
class CWp7PushUserManage : public CStaticNodeManage<CWp7PushUser>
{
public:
	CWp7PushUserManage();
	~CWp7PushUserManage();
...
protected:
	// 更新设置新节点
	const int UpdateSetNode(const int HeadIndex, const CWp7PushUser &Node);
	// 节点(组)是否有效
...
public:
	...
	// 获取用户
	CWp7PushUser* GetUser(const unsigned int);
	// 更新占用的用户数属性
	const short UpdateOccupyProperty(const unsigned int);
	// 更新已经占用的预警资源属性
	const short UpdateUseWarnProperty(const unsigned int);
	// 从数据库加载用户
	const int TryReadUserFromDb(const char *, const unsigned char, CWp7PushUser*);
	// 同步用户基本信息头
...
#endif	/* _INCLUDE_WP7_USER_MANAGE_H */
