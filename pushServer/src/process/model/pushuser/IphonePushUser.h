#ifndef _INCLUDE_IPHONE_PUSH_USER_H
#define _INCLUDE_IPHONE_PUSH_USER_H

#include "PushUser.h"

class CIphonePushUser : public CPushUser
{
public:
	CIphonePushUser(void);
	CIphonePushUser(const UserLogInfor*, const unsigned int);
	~CIphonePushUser(void);

// 扩充数据
protected:
	char m_strPushToken[MAX_IOS_PUSH_TOKEN_LEN + 1];				// 推送令牌
...
private:
	...
	int ParseReq1000(const sub_head *, const UserLogInfor*, 		// 处理1000请求
					void *data, const unsigned int, CBuffWriter*);
	bool AddIphoneEWarningMsg(const PushMsg*, Payload*);			// 以iphone的记录格式添加预警记录
	bool AddIphonePublicNoticeMsg(const PushMsg*,Payload*);			// 以iphone的记录格式生成公共消息记录
	bool AddIphoneMineMsg(const PushMsg*, Payload*);				// 以iphone的记录格式生成信息地雷记录
...
#endif	/* _INCLUDE_IPHONE_PUSH_USER_H */
