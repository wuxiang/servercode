...
CIphonePushUser::CIphonePushUser(void)
	: CPushUser()
{
	bzero(m_strPushToken, MAX_IOS_PUSH_TOKEN_LEN + 1);
}

CIphonePushUser::CIphonePushUser(const UserLogInfor *LogInfo, const unsigned int ExpandInfo)
	: CPushUser(LogInfo)
{
...
// 设置推送令牌
int
CIphonePushUser::SetUserPushToken(const char *token)
{
	StrCopy(m_strPushToken, token, MAX_IOS_PUSH_TOKEN_LEN);
	return 0;
}

// 获取推送令牌
void
CIphonePushUser::GetUserPushToken(char *token, const int MaxLen)
{
...
// 执行节点股价预警计算
int
CIphonePushUser::ExecuteStkPriceWarningCal(const unsigned int UserMapID)
{
	// 死亡用户无需再扫描
...
// 执行自选股新闻相关扫描
int
CIphonePushUser::ExecutePrivateNoticeScan(const unsigned int UserMapID)
{
	// 死亡用户无需再扫描
	const int iCurrentActiveP = GetActiveProperty();
	if (PU_DEAD == iCurrentActiveP)
...
// 执行公共新闻公告相关扫描
int
CIphonePushUser::ExecutePublicNoticeScan(const unsigned int UserMapID)
{
	// 死亡用户无需再扫描
	const int iCurrentActiveP = GetActiveProperty();
	if (PU_DEAD == iCurrentActiveP)
...
// 以iphone的记录格式添加预警记录
bool
CIphonePushUser::AddIphoneEWarningMsg(const PushMsg *pPushMsgRec, Payload *writer)
{
	// 生成消息
	...
	strncpy(writer->message, pMsg, PL_MSG_LEN);

    // This is the red numbered badge that appears over the Icon
    writer->badgeNumber = 1;

    // This is the Caption of the Action key on the Dialog that appears
    bzero(writer->actionKeyCaption, PL_ACTIONKEYCAPTION_LEN);

    // dictionary key-value pairs with user-content
    strncpy(writer->dictKey[0], "StkCode", PL_DICT_KEY_LEN);
    strncpy(writer->dictValue[0], strCode.c_str(), PL_DICT_VALUE_LEN);
    
    char *pType = "304";
    strncpy(writer->dictKey[1], "redirect", PL_DICT_KEY_LEN);
    strncpy(writer->dictValue[1], pType, PL_DICT_VALUE_LEN);

    return true;
}

// 以iphone的记录格式生成公共消息记录
bool
CIphonePushUser::AddIphonePublicNoticeMsg(const PushMsg *pPushMsgRec, Payload *writer)
{
	if (NULL == pPushMsgRec)
	{
	...
	if (NOTICE_NORMAL == cType)
	{
	    ...
	    // dictionary key-value pairs with user-content    
	    char *pType = "310";
	    strncpy(writer->dictKey[0], "redirect", PL_DICT_KEY_LEN);
    	strncpy(writer->dictValue[0], pType, PL_DICT_VALUE_LEN);
	...
	else if (NOTICE_EXTENSION == cType)
	{
	    ...
	    strncpy(writer->dictKey[1], "URL", PL_DICT_KEY_LEN);
	    strncpy(writer->dictValue[1], MsgRecord.strURL, PL_DICT_VALUE_LEN);
	    return true;
	}
...
// 以iphone的记录格式生成信息地雷记录
bool
CIphonePushUser::AddIphoneMineMsg(const PushMsg *pPushMsgRec, Payload *writer)
{
	// 生成消息
	char cReceiveBuf[256] = {0};
	const char *pMsg = NULL;
	...
	if (PTE_INFOMINE == pPushMsgRec->MsgType)
	{
		...
		else if (INFOMINE_STATISTIC == pPushMsgRec->MsgSubType)
		{
			...
			pMsg = ChangeToInfoMineMsg(&InfoMineMsg, cReceiveBuf, 256);
			// This is the red numbered badge that appears over the Icon
    		writer->badgeNumber = pPushMsgRec->LatestValue;
		}
	}
    ...
    char *pType = "311";
    strncpy(writer->dictKey[1], "redirect", PL_DICT_KEY_LEN);
    strncpy(writer->dictValue[1], pType, strlen(pType));
...
// 处理306请求
int
CIphonePushUser::ParseReq306(const sub_head *pCmdHead, const UserLogInfor *pUlf,
...
// 处理1000请求
int
CIphonePushUser::ParseReq1000(const sub_head *pCmdHead, const UserLogInfor *pUlf,
		void *data, const unsigned int length, CBuffWriter *writer)
{
	int iRes = 0;
	CBuffReader reader(data, length, 0);
	...
	cResMark = 0;
	memcpy(&subHead, pCmdHead, sizeof(sub_head));
	subHead.sub_length = sizeof(char) + sizeof(char) + sizeof(CIphonePushUser);
	writer->Push_back(&subHead, sizeof(sub_head));
	writer->Push_back(&cResMark, sizeof(char));
	writer->Push_back(&pUlf->m_cPlatformCode, sizeof(char));
	writer->Push_back(this, sizeof(CIphonePushUser));
...
// 检查用户节点的运行状态
const int
CIphonePushUser::CheckUserAliveStatus(const time_t NowTime)
{
...
// 处理用户请求
int
CIphonePushUser::ProcessUserRequest(const sub_head *pCmdHead,  const UserLogInfor *pUlf, void *data,
...
// 发送预警消息
const int 
CIphonePushUser::SendEWarningMsg(const unsigned int UserMapID)
{
	...
	for (unsigned int i = m_iPushWarningNum; i < m_iPushWarningTotal; i++)
	{
		...
		if (!AddIphoneEWarningMsg(pHis, &payload))
		{
			DEBUG("AddIphoneEWarningMsg failed.");
			break;
		}
		uCurrentPushCrc = i;
		
		if (0 == pCurrentThread->UserExecuteSend(m_cPlatformCode, m_strPushToken, &payload))
		{
			iSendCount++;
			DEBUG("User[%s] send apple push msg", GetUserID());
...
// 发送信息地雷
const int
CIphonePushUser::SendInfoMineMsg(const unsigned int UserMapID)
{
	...
	for (unsigned int i = m_iInfoMineNum; i < m_iInfoMineTotal; i++)
	{
		...
		bzero(&payload, sizeof(Payload));
		if (!AddIphoneMineMsg(pHis, &payload))
		{
			DEBUG("AddIphoneMineMsg failed.");
			break;
		}
		uCurrentPushCrc = i;
		...
		{
			iSendCount++;
			DEBUG("User[%s] send apple InfoMine msg", GetUserID());
		...
		{
			TRACE("Send  apple InfoMine error");
		}
	}
...
// 发送公告
const int 
CIphonePushUser::SendNoticeMsg(const unsigned int UserMapID)
{
	...
	{		
		...
		bzero(&payload, sizeof(Payload));
		if (!AddIphonePublicNoticeMsg(pHis, &payload))
		{
			DEBUG("AddIphonePublicNoticeMsg failed.");
			break;
		}
		uCurrentPushCrc = i;
		...
		{
			iSendCount++;
			DEBUG("User[%s] send apple Notice msg", GetUserID());
...
// 发送消息
int
CIphonePushUser::SendOuterMsg(const unsigned int UserMapID, const unsigned char MsgType)
{
	switch(MsgType)
	{
		case PTE_INFOMINE:
...
// 执行发送
int
CIphonePushUser::ExecuteSend(const unsigned int UserMapID, const unsigned char PushType)
{	
...
// 执行自选股信息地雷扫描
const int
CIphonePushUser::ExecuteSelfStkInfoMine(const unsigned int UserMapID)
{
	// 死亡用户无需再扫描
	const int iCurrentActiveP = GetActiveProperty();
	if (PU_DEAD == iCurrentActiveP)
...
// 执行自选股信息地雷统计
const int
CIphonePushUser::ProcessSelfStkInfoMineStatis(const unsigned int UserMapID)
{
	CSelfSelectStock *pSelfSelectStock = CSelfSelectStockManage::GetPlatformManage(m_cPlatformCode);
	const int iRecCount = pSelfSelectStock->GetSelfCodeCount(UserMapID);
